...
export class RemoteBase extends EventEmitter {
	...
	public async initWpmemAssertionLogpoints() {
		...
		const watchpoints = InstrumentationParser.createWatchPoints(watchPointLines);
		this.setWPMEMArray(watchpoints);

		// ASSERTIONs
		// Set assertion breakpoints
		const assertionLines = Labels.getAssertionLines();
		const assertionsArray = InstrumentationParser.createAssertions(assertionLines);
		this.setASSERTIONArray(assertionsArray);

		// LOGPOINTs
		const logPointLines = Labels.getLogPointLines();
		const logPointsMap = InstrumentationParser.createLogPoints(logPointLines, this, msg => this.emit('warning', msg));
	 ...
	 * @param vars The system variables to load. 'sysvars-zx16k', or 'sysvars-zx48k'.
	 */
	public async loadSysVars(vars: LoadSysVar): Promise<void> {
		if (vars === 'sysvars-zx16k' || vars === 'sysvars-zx48k') {
			// ZX Spectrum.
			// Clear the screen.
			this.writeMemoryDump(0x4000, new Uint8Array(0x1800)); // Is initialized with zeroes
			// Clear the attribute memory, black on white.
			this.writeMemoryDump(0x5800, new Uint8Array(0x0300).fill(0x38));
			// Sysvars
			const RAMTOP = (vars === 'sysvars-zx16k') ? 0x7F57 : 0xFF57;
			const sysvars = [
				/* 5C00: */
				...
				/* 5C70: */
				0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
				0x26, 0x05, 0x00, 0x58, 0xFF, 0x00, 0x00, 0x21,
				/* 5C80: */
				...
				0x00, 0x00,
				/*5CB2 = RAMTOP */
				RAMTOP & 0xFF, RAMTOP >> 8,
				/* 5CB4: */
				0xFF, 0xFF, 0xF4, 0x09,
				0xA8, 0x10, 0x4B, 0xF4, 0x09, 0xC4, 0x15, 0x53,
				/* 5CC0: */
				0x81, 0x0F, 0xC4, 0x15, 0x52, 0xF4, 0x09, 0xC4,
				0x15, 0x50, 0x80, 0x80, 0x0D, 0x80
			];
			this.writeMemoryDump(0x5C00, new Uint8Array(sysvars));
		}
	}
	...
	*/
	...
	public async readBankMemoryDump(bank: number, offset: number, size: number): Promise<Uint8Array> {
		throw Error(`Reading from a memory bank is not supported by '${this.remoteType}'.`);
	}
	...
	public async writeBankMemoryDump(bank: number, offset: number, dataArray: Uint8Array): Promise<void> {
		throw Error(`Writing to a memory bank is not supported by '${this.remoteType}'.`);
	}
