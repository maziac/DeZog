...
export class DebugSessionClass extends DebugSession {
	...
	// Executes the debug console commands (e.g. "-help", "-md").
	protected consoleCommands = new DebugConsoleCommands({
		sendEvent: event => this.sendEvent(event),
		stateSave: stateName => this.stateSave(stateName),
		stateRestore: stateName => this.stateRestore(stateName),
		loadLabels: settingsLaunch => this.loadLabels(settingsLaunch)
	});
					...
					(async () => {
						...
						try {
							...
							for (const cmd of Settings.launch.commandsAfterLaunch) {
								...
								try {
									const outText = await this.consoleCommands.evaluateCommand(cmd);
	...
	protected async evaluateRequest(response: DebugProtocol.EvaluateResponse, args: DebugProtocol.EvaluateArguments): Promise<void> {
		...
		switch (args.context) {
			...
			case 'repl': {
				let text;
				try {
					text = await this.consoleCommands.evaluateCommand(expression);
