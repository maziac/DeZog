...
import {Bytes} from '../src/misc/bytes';
...
suite('Bytes', () => {
	...
	suite('getUintFromMemory', () => {
		...
		suite('little endian', () => {
			...
			test('byte', async () => {
				...
				for (let i = 0; i < values.length; i++) {
					const val = Bytes.getUintFromMemory(mem, i, 1, true);
			...
			test('word', async () => {
				...
				for (let i = 0; i < values.length - 1; i++) {
					const val = Bytes.getUintFromMemory(mem, i, 2, true);
			...
			test('count = 5', async () => {
				...
				const val = Bytes.getUintFromMemory(mem, 1, 5, true);
		...
		suite('big endian', () => {
			...
			test('byte', async () => {
				...
				for (let i = 0; i < values.length; i++) {
					const val = Bytes.getUintFromMemory(mem, i, 1, false);
					assert.equal(val, values[i]);
				}
			});
			...
			test('word', async () => {
				...
				for (let i = 0; i < values.length - 1; i++) {
					const val = Bytes.getUintFromMemory(mem, i, 2, false);
			...
			test('count = 5', async () => {
				...
				const val = Bytes.getUintFromMemory(mem, 1, 5, false);
	...
	suite('setUintToMemory', () => {
		...
		suite('big endian', () => {
			...
			test('word', () => {
				const memory = new Uint8Array([0, 0, 0, 0]);
				Bytes.setUintToMemory(0x1234, memory, 1, 2, false);
				assert.deepEqual(Array.from(memory), [0, 0x12, 0x34, 0]);
			});

			test('count = 3', () => {
				const memory = new Uint8Array(3);
				Bytes.setUintToMemory(0x123456, memory, 0, 3, false);
	...
	suite('getWord / setWord', () => {
		...
		test('setWord little endian', () => {
			...
			Bytes.setWord(buffer, 1, 0x1234);
			assert.deepEqual(Array.from(buffer), [0, 0x34, 0x12, 0]);
		});

		test('getWord little endian', () => {
			const buffer = Buffer.from([0x11, 0x34, 0x12]);
			assert.equal(Bytes.getWord(buffer, 1), 0x1234);
			assert.equal(Bytes.getWord(buffer, 0), 0x3411);
		});

		test('round trip', () => {
			const buffer = Buffer.alloc(2);
			Bytes.setWord(buffer, 0, 0xFFFE);
			assert.equal(Bytes.getWord(buffer, 0), 0xFFFE);
		});
	});
	...
	suite('getStringFromBuffer', () => {
		...
		test('stops at 0', () => {
			...
			assert.equal(Bytes.getStringFromBuffer(buffer, 0), 'AB');
		});

		test('start index', () => {
			const buffer = Buffer.from([0x41, 0x42, 0x43, 0]);
			assert.equal(Bytes.getStringFromBuffer(buffer, 1), 'BC');
		});

		test('no terminating 0', () => {
			const buffer = Buffer.from([0x41, 0x42, 0x43]);
			assert.equal(Bytes.getStringFromBuffer(buffer, 0), 'ABC');
		});

		test('empty', () => {
			const buffer = Buffer.from([0x41, 0, 0x42]);
			assert.equal(Bytes.getStringFromBuffer(buffer, 1), '');
		});
	});
});
